...
<li
	...
	class="mb-0.5 rounded-xs px-3 py-2 last:mb-0"
>
	<button
		type="button"
		onclick={() => {
		...
		class="items-centertransition flex w-full cursor-pointer text-slate-200 duration-150 hover:text-slate-200"
	>
	...
	{#if stateExpanded}
		...
		>
			<ul class="mt-1 pl-7">
				{#each propData.arraySubmenus as currentSubmenu (currentSubmenu.stringName)}
					<li class="mb-1 last:mb-0">
						...
						>
							<div
								class="flex items-center justify-between py-1 text-sm font-medium whitespace-normal duration-200 lg:opacity-0 2xl:opacity-100"
								class:lg:opacity-100={propSidebarExpanded}
								class:lg:opacity-0={!propSidebarExpanded}
							>
								...
								{#if currentSubmenu.stringBadge !== undefined}
									<div
										...
										class="mr-1 flex h-5 items-center rounded-sm px-2 text-xs font-medium"
									>
