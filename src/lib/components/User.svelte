...
<div use:functionClickOutside={() => (stateExpanded = false)} class="relative inline-flex">
	<button
		...
		class="group inline-flex cursor-pointer items-center justify-center"
		aria-haspopup="true"
		aria-expanded="false"
	>
		<img
		...
		<div class="flex items-center truncate">
			<span
				class="!md:text-sm ml-2 truncate font-medium group-hover:text-slate-800 dark:text-slate-300 dark:group-hover:text-slate-200"
			>
				{propTitle}
			</span>
	...
	{#if stateExpanded}
		...
		>
			<div>
				<div class="mb-1 border-b border-slate-200 px-3 pt-0.5 pb-2 dark:border-slate-700">
				...
				<ul>
					{#each propUserMenuArray as currentUserMenu (currentUserMenu.stringText)}
						<li>
							<a
				...
				{#if propButtonText !== ''}
					<div
						...
						class="mt-1 flex justify-center border-slate-200 px-3 pt-2 pb-0.5 dark:border-slate-700"
					>
						<button
							onclick={(event) => {
							...
							class="block cursor-pointer rounded-sm border border-zinc-700 bg-zinc-100 px-3 py-1 text-center font-semibold whitespace-nowrap text-zinc-700 hover:border-transparent hover:bg-zinc-300"
						>
							{propButtonText}
						</button>
					</div>
